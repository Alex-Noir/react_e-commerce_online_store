...
class ContextProvider extends Component {
  ...
  addToCart(id, inputValue, hasDiscount) {
    ...
    const idOfCartListItem = cartList.findIndex(
    ...
    if (inputValue >= 1 && arrayOfIds.indexOf(id) === -1) {
      ...
      if (hasDiscount) {
        item.totalPrice = Number.parseFloat(
          (price * amountInCart * discount).toFixed(2)
        );
      } else {
  ...
  deleteItemFromCart(id) {
    ...
    const cartList = this.state.cartList;
    const cartListItemIndex = cartList.findIndex(
      (cartListItem) => cartListItem.id === id
    );
  ...
  evaluateTotalPrice() {
    ...
    this.state.cartList.map(
      (cartListItem) => (cartSubTotalPrice += cartListItem.totalPrice)
    );
